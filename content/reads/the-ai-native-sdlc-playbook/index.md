...
不少组织已经能用 AI 以一年前想都不敢想的速度写代码，但代码周围的那套流程并没有跟着变快。

许多工程团队仍在沿用原先那套审批关卡、评审、交接和策略，用 [Claude Code](https://claude.com/product/claude-code) 这类 agentic 编码方案换来的提速，就这样被抵消掉了。

软件开发生命周期（SDLC）是把软件从一个想法推进到生产环境的过程。大多数组织都在跑着大致相同的六个阶段：规划、设计、构建、测试、部署、维护。传统上，每个阶段都由不同角色负责，各管一段：产品经理写需求，技术架构师把需求转成设计，工程师实现设计，受监管企业里的 QA 团队负责验证，发布团队负责上线，运维团队盯着运行状况。工作在阶段之间流转，靠的是文档、工单和签字确认。

传统的 SDLC 流程繁重，为的是保证每一步都有人负责、都在掌控之中。但它是为一个「编写和实现代码是最耗时、最昂贵的阶段」的年代设计的，这个前提现在已经不成立了。PRD、估算仪式、产品安全评审这些环节之所以存在，就是为了在可能拖上几周、几个月甚至几个季度的开发周期里，硬逼各方对齐。

传统 SDLC 还有一个特点：每道控制措施都默认由人来执行。而那些收获最多的组织，已经围绕 agentic AI 现在能做到的事重建了流程，并始终把人留在其中。这份指南介绍的是 Applied AI 团队在 SDLC 每个阶段集成 Claude 的一组最佳实践，来自我们与客户合作的经验，目的是加快开发、让流程转得更快。

当代码不再是瓶颈、构建阶段的速度超出传统 SDLC 能容纳的范围时，会出现三件事：

- 瓶颈转移到构建阶段前后的环节，主要是规划、评审/测试和部署，它们仍然以人类的速度运转。
- 控制措施不再符合现实，变得难以应付。代码由人编写时，逐行人工审查是合理的；一旦 Agent 产出了 diff 的大部分内容，人工审查就跟不上了。
- 治理成本上升，因为例外情况仍然要靠每周、每月才开一次的会议和委员会来处理。

![](code-is-no-longer-the-bottleneck.png)
*构建不再是制约因素，它周围那些以人类速度运转的环节才是。以人类速度运转的阶段维持原有时长，构建则被压缩到数小时。*

拿安全瓶颈举个例子。安全团队的规模是按人类产出来配置的，所以当 Agent 让代码产出成倍增长时，要么评审队列越积越长，要么代码在评审不足的情况下上线。受监管的组织两种结果都无法接受，安全和策略检查必须跟上 Agent 的节奏才行。

传统 SDLC 需要经历一次和「实现」环节同等力度的转型，才能既保住 agentic AI 带来的效率，又守住安全。

## 什么是 AI 原生 SDLC？

AI 原生 SDLC 是一次重新构想：旧有的控制目标不变，但换成新的执行方式。流程不再是线性的，而是变成一个循环，AI 被嵌入每一个节点。它倡导自动化的交接、自动触发的后续打法，正好解决传统 SDLC 各阶段之间那些手工、笨拙的交接。

你也会听到有人把这种转变叫做 agentic SDLC、AI SDLC，或者干脆叫 agentic 软件开发。叫法不同，说的是同一件事。

![](what-is-an-ai-native-sdlc.png)

### AI 原生 SDLC 六个阶段的转变

下表是 Claude 支撑下的两个极端：传统 SDLC 与 AI 原生 SDLC。大多数组织都落在两列之间的某个位置。

| 阶段 | 传统 SDLC | AI 原生 SDLC |
| --- | --- | --- |
| 规划 | 需求由委员会收集，靠研讨会和签字确认来提炼，再手工整理成文档 | Claude 直接从源头把痛点综合起来，写进 `intent.md`，既适合人读，也适合机器执行 |
| 设计 | 由分析师撰写规格，再由设计师解读 | 需求与设计被压缩到和 Agent 的一次会话里，由编成 Skill、纳入 git 版本管理的标准来引导 |
| 构建 | 测试和代码手动编写，文档等到主要开发完成之后才补 | 测试和代码由 AI 生成，机构知识以纳入版本管理、机器可读的 `CLAUDE.md` 文件和 Skill 的形式维护 |
| 测试 | 在阶段边界设置 QA 关卡 | 贯穿实现过程的持续评估 |
| 部署 | 人工逐行审查代码，治理发生在评审周期里，还常常不一致 | 多层 agentic 审查，人工审查只为受监管和关键代码保留。治理在 AI 行动时实时执行，用 Hook 充当审批关卡 |
| 维护 | 人工监控生产环境以发现问题 | Agent 监控实时部署。任何被突破的控制带都会被诊断，并写回循环中，成为新的 `intent.md` |

贯穿右栏的主线是「已提交的产物」。每个阶段结束时，都会把一个产物写进版本控制（`intent.md`、`spec.md`、`plan.md`、diff 及其测试、带评审意见的 PR，以及事故记录），下一个阶段就从读它开始。早期阶段的主要产物是 .md 文件，因为产品负责人和 Agent 都能读、也都能对同一份文件下手。到了构建阶段，产物就变成了代码及其记录。这条提交链同时就是审计轨迹：谁提出了什么，Agent 产出了什么，谁批准了它。

需要判断的决策仍然都要由人负责。在 agentic SDLC 里，人的注意力会转移到那些必须接受评审的产物上。

## 打法

打法是这份指南的核心，分成六个非线性的阶段（规划、设计、构建、测试、部署、维护），合起来覆盖整个生命周期。
...
这些步骤是模块化的，组织可以根据自身需求，在不同时间优先改造不同的阶段。每个打法都会在「先决条件」下列出它的依赖项，依赖关系图对此做了进一步的说明。

一个阶段以提交某个产物收尾，这个提交又会启动下一个阶段。被接受的 `intent.md` 触发需求与设计环节，被批准的 `spec.md` 触发 plan mode，被合并的 PR 触发流水线，生产环境中被突破的控制带则写出下一个 `intent.md`，如此循环往复。

起初，你得手工为每一步写 Prompt；最终形态是一个循环，每个被接受的产物都触发下一道关卡。人的注意力集中在关卡上，评审 Agent 标记出的内容，而不是从头启动每一个阶段。

![](plays.png)
*图中按阶段列出了各个打法，箭头表示采纳它们的顺序，两者并不相同。你可以从任何一个基础打法开始，它没有任何箭头指向，所以不需要前置条件。其余打法，指向它的箭头所代表的打法，就是要先于它采纳的。*

## 规划

### 以 intent.md 的形式捕获

启动软件开发流程的 `intent.md` 可以从不同途径进来：有人冒出一个想法，有人提了张工单，也可能是一场事故通过告警暴露出来（见第六阶段：维护）。

有了想法之后，他会和 Claude 一起头脑风暴，产出一份 markdown 原型规格。换成传统 SDLC，这个人接下来还得说服产品团队里的某个人，跟他一起、或者替他，把这个想法写成文档。

Claude 生成的这份原型规格既适合人读，也纳入了版本控制，还能立刻被下一个阶段使用。它会被保存为 `intent.md`。

不管这个 intent 来自事件触发还是来自 Agent，后续步骤都一样：产品负责人在提交之前，评审并修正由 Agent 写出的 `intent.md`。

这套设置对平台或工程团队来说是一次性的工作，得由一位技术团队成员搭好 intent 仓库，并决定谁可以往里写，因为贡献者会来自整个组织的不同部门。

仓库建好之后，没有 git 经验的贡献者不用直接碰 git。而是由一个接到版本控制系统的连接器（比如 GitHub），让 Claude 从 claude.ai 或 Cowork 替他们把 markdown 文件提交上去。

#### 如何执行

1. 发起人用自己的话向 Claude 描述问题：现在做不到什么、这个想法会影响到谁、更好的结果长什么样，或者哪些内容不在范围内。措辞不必正式。
2. 持续头脑风暴，直到想法变得具体。Claude 会提出分析师会问的问题：范围、用户、约束，以及成功的标准是什么。
3. 让 Claude 按组织的模板把结果写成 `intent.md`。这个模板可以由技术团队成员编码成一个 Skill，再由一位负责人签字确认。文档里可以包含问题、提议的成果、受影响的用户和系统、约束，以及待解决的问题。
4. 发起人修正 Claude 理解有误的地方。
5. 把 `intent.md` 提交到共享仓库。作者和时间戳一并记录在案，产品负责人从那里接手这个想法。
...
证据就是那份已提交的 `intent.md`，它记录了作者、时间戳和完整的修订历史，都存在 intent 仓库的 git 历史里。批准由产品负责人负责；把 intent 送进第二阶段（设计）之前的接受或拒绝，则记录为一次合并或关闭评审。

## 设计

### 需求与设计

一旦得到产品负责人批准，Claude 就拿着这份被接受的 `intent.md`，产出一份需求与设计规格。整个过程由组织围绕品牌、安全、合规和 UX 制定的 [Skill](https://code.claude.com/docs/en/skills) 来引导。

产品负责人负责评审这份规格，但不亲自撰写。这个流程的目标是产出一份工程团队可以据此制定计划的规格，并标注出需要关注的地方。

前端工作是最清晰的例子。一旦 `intent.md` 被接受，产品负责人就根据它在 [Claude Design](https://claude.com/product/design)（beta）里搭建设计原型，反复迭代，然后导出到 Claude Code 里进行构建。

#### 如何执行

1. 产品负责人开启一个会话，让组织的 Skill 可用，并附上 `intent.md`。
2. 产品负责人的 Prompt 指向 `intent.md`，指明约束条件，并要求标注出关注点。先手工执行，再把这一步固化成组织级的 slash 命令。之后，把 intent 仓库中 `intent.md` 被接受这件事当作触发器，用一个合并时触发的非交互式作业、在加载了组织 Skill 的情况下运行这一环节，并把 `spec.md` 作为拉取请求提交（搭管道的事由第五阶段「部署」里的 CI/CD 打法负责）。从这以后，产品负责人的第一次介入就是评审。
3. 同一位产品负责人对照最初的想法来评审这份规格：它是否解决了所陈述的问题？`intent.md` 里待解决的问题是被解答了，还是被延续了下来？
4. 先处理被标注出来的关注点，它们正是分析师原本会升级上报的问题。产品负责人要在工程团队看到规格之前，和相应的策略负责人逐一解决这些点。
5. 把 `spec.md` 与 `intent.md` 一并提交。这对文件记录了提出的是什么、决定了的是什么。
6. 产品负责人决定这份规格和 intent 是否推进到构建阶段，对于组织认定为较高风险的事项，则咨询技术负责人。这个决定始终由一位人类团队成员做出，而接受这份规格，正是启动第三阶段（构建）中 plan mode 打法的信号。
...
现行策略不再是拖到几周后的评审才被发现，而是在规格写出来的时候就被读取、被应用。组织的 Skill 作为约束施加在规格上；这份规格、生成它的 Prompt，以及当时生效的 Skill 版本，都记录在版本控制里。产品负责人签署确认规格，并把标注出的关注点转交给指定的策略负责人。

## 构建

### Claude Code plan mode 作为默认起点

工程师在 [plan mode](https://code.claude.com/docs/en/permission-modes) 下开启 Claude Code 会话，把第二阶段（设计）里已获批准的 `spec.md` 交给 Claude，让它反过来向自己提问，反复迭代计划，直到工程师满意。

#### 如何执行

1. 工程师在 plan mode 下与 Claude 开启会话。
2. 工程师把 `intent.md` 和 `spec.md` 交给 Claude，要求它给出一个实现计划，指明哪些文件会变化、工作的顺序，以及用于验证的测试。
3. 通过追问来审视这份计划：这次变更可能破坏什么？哪一步风险最大？Claude 还有哪些没有选择的方案？
4. 反复迭代，直到一个从未看过这段对话的工程师，仅凭这份计划就能实现这次变更。
5. 把已获批准的计划作为 `plan.md` 提交。这份计划进入审计轨迹，PR 评审打法（第五阶段：部署）据此核验最终的 diff。
6. 接受这份计划，让 Claude 去实现。有了扎实的计划，实现往往一遍就能完成。
7. 当实现偏离计划时，在同一次提交里更新 `plan.md`。可以考虑用一个 Hook 来强制两者保持同步。
...
设计评审发生在任何代码生成之前，此时改变方向还只是编辑一份文档而已。plan mode 本身就强制了这一点，因为在工程师接受计划之前，Claude 无法编辑文件。计划及其修订会连同接受者一起被记录下来。常规变更由工程师批准，组织认定为较高风险的事项则交由技术负责人或架构师处理。

### 在 auto mode 下运行 Claude Code

Claude Code 也可以在 auto mode 下运行：工程师批准计划，一旦满意并完成迭代，Claude 就会在不逐次确认编辑的情况下应用每一处改动。随着后续打法中的护栏逐渐成熟（一份调校好的 `CLAUDE.md`、编码了策略的 Skill、阻止不安全操作的 Hook，以及一个 Claude 可以运行的测试套件），auto-accept 会成为常规工作的默认方式：一份紧凑的 `spec.md`、较小的改动影响范围，以及已有测试覆盖的代码。

这种转变意味着，用户不再盯着 Agent 逐次编辑、审查每一步操作，而是在更长的自主会话之后评审产物。配合 worktree 使用，auto-accept 模式还能进一步在个人和团队层面并行化，这对于自主运行 SDLC、实现第六阶段（维护）所描述的闭环至关重要。

### CLAUDE.md

[`CLAUDE.md`](https://code.claude.com/docs/en/memory) 给 Claude 的是新成员需要的那部分上下文：约定、命令、架构，以及团队最容易犯的错。过去放在人们脑子里和 wiki 上的知识，变成了一份 Agent 每次开会话都会读的文件，由整个团队一起维护，每犯一次错就迭代一次。
...
`CLAUDE.md` 纳入版本控制，因此 Agent 所遵循的指令是可评审、可审计的。团队约定通过这份文件来落实，对它的变更记录在 git 历史里，代码负责人在 PR 评审中批准这些变更。

### 把 Skill 作为机构知识

Skill 是组织把机构知识变得可操作的方式。这些指令明确、纳入版本控制、被广泛复用，而且策略一变就会集中更新。经验法则是：为必须一致应用的机构知识写 Skill；本来该放进 `CLAUDE.md` 或 Prompt 的内容，就别写成 Skill。

#### 如何执行

1. 挑选一条目前执行得不一致的知识。它可以是安全标准、API 设计约定，或品牌规则。
2. 把它写成一个 Skill，也就是一个包含 `SKILL.md` 的文件夹，其 frontmatter 说明何时触发，正文说明该做什么。由工程师根据策略负责人提供的唯一事实来源来编写，Claude 从旁协助。
...
Skill 是一种控制手段，尽管是建议性质的。它让 Claude 在编写代码时更有可能应用该策略，但没有任何东西能强制某个会话遵守它。对于必须始终成立的策略，需要在 Skill 背后有确定性的机制，例如一个阻止该操作的 Hook，或在 PR 阶段重新核验该策略的评审环节。Skill 让违规变得罕见，Hook 让违规几乎不可能发生。Skill 的调用会记录在会话追踪中，策略负责人也会像评审代码一样评审 Skill 的变更。

### 用 Hook 作为构建期护栏

Skill 是一种建议性质的控制，[hook](https://code.claude.com/docs/en/hooks) 则是它背后的确定性机制。实现过程中，Claude 的大多数操作都是文件编辑和 shell 命令，因此构建阶段是 Hook 最常被触发的地方。
...
对于任何必须无例外成立的策略，都要用 Hook 来兜底。Hook 会在每个与之匹配的操作上运行，因此构建阶段的 Hook 应当快速，并只针对发生变化的文件。完整测试套件这类较重的检查，则应放在提交或 PR 环节。
...
并行会话提高了工程师能同时推进的任务数量，Subagent 让每个会话专注于自己的任务。工程师的工作就是驾驭和评审所有这些会话。

#### 如何执行

1. 工程师利用 plan mode 打法（第三阶段：构建）中的计划，把工作拆分成涉及不同文件的任务，看清哪些工作是相互独立的。涉及相同文件的任务放在单个会话里，按顺序执行。
2. 每个并行任务都有自己的 worktree，例如在一个终端里运行 `claude --worktree feature-auth`，在另一个终端里运行 `claude --worktree fix-rate-limit`。worktree 是各自分支上的独立检出，能避免会话在文件上发生冲突。
3. 两到三个会话是一个合理的起点。实际上限是一个人能妥善评审多少条工作流，所以只有当评审跟得上时，才增加会话。
4. 把重复性的工作改造成 Subagent，它们定义在 `.claude/agents/` 目录下的 markdown 文件中，每个都包含名称、使用时机说明，以及可以接触的工具。例子包括：在主要 Agent 完成后剥离不必要复杂度的代码简化器、运行应用并检查行为的验证器、探索代码库并回报结果而不会淹没主上下文的研究器。把这些定义提交到 git 里，让整个团队共享。
...
不要把反馈回路与验证器 Subagent（第三阶段：构建）混为一谈。反馈回路贯穿整个任务，随着工作推进反复运行；验证器 Subagent 则是在会话认为工作已经完成之后，用一个全新的上下文窗口来执行最终检查。这样一来，结论就不会被产出代码的那些假设所影响。

#### 如何执行

1. 如果今天检查工作需要一连串命令和一定的环境知识，就把它封装成单个目标，例如 "make test" 或 "npm test"，在失败时以非零状态退出。
2. 在 `CLAUDE.md` 的 Commands 部分，为每条命令列出一个健康输出的示例。
3. 明确一个目标并让它可量化，这样 Claude 无需询问你就能检查工作，例如：「test_status.py 中的所有测试都通过」、「截图与所附的原型一致」，或「该接口带着新字段返回 200」。
...
7. 最后，这个回路本身也需要保护，因为正在修复代码的 Agent 绝不能有能力削弱对这段代码的检查。一个在修复任务期间阻止编辑测试文件的 Hook 就能做到这一点。另一种办法是在评审中检查 diff，拒绝任何改动测试的变更。
...
评估（eval）是 AI 原生世界里阶段关卡式 QA 的对应物。在实践中，它指的是每当 Agent 的配置发生变化时就会运行的一套测试。当换入一个新模型或改写一个 Prompt 时，评估套件会说明这个 Agent 是否仍以同样的标准完成工作。

评估应当被看作一个持续演进的套件。随着模型的改进，那些曾经能区分好坏的用例会渐渐失去区分度，必须加入由持续监控产生的新用例。
...
5. 每一起生产事故都要有一个评估，由负责该事故的团队编写，并作为回归测试留在套件里。
...
Claude 既给出评审，也接受评审。它依据组织的策略评审进来的 PR，并处理自己 PR 上的评审意见。这让工程师能够在 PR 评审中专注于行为本身，归结起来就是判断意图和风险。
...
构建阶段把 Hook 用作护栏，在没有人工参与的情况下允许或阻止操作（第三阶段：构建）。Hook 还可以询问，暂停操作直到某个指定的人批准，这正是发布把关所需要的。
...
例如，一个持续运行的监控 Agent 可以在 bug 工单提出之后，创建一份 `intent.md`，然后依次流经需求、计划、构建、测试和评审阶段。第六阶段（维护）是无头运行的，阶段之间有一个独立的置信关卡：一个确定性检查，或一个对抗式审查 Agent，用来决定上一阶段的产出是继续推进，还是升级给人工处理。
...
6. 对于超出单个补丁范围的任何问题，例如架构上的薄弱点或跨服务重复出现的模式，就以第一阶段的格式写成 `intent.md`，并从规划阶段开始处理。
...
修复是通过 PR 评审关卡和分支保护到达生产环境的，而不是来自扫描本身。Claude Security 是对现有静态分析和依赖扫描的增强。确定性检查继续留在 CI 中，而模型驱动的扫描则覆盖那些依赖上下文、而这些检查本就不擅长的漏洞。
...
模型和 Harness 已经更加先进，让组织不仅能够改造自己编写代码的方式，还能改造整个软件开发生命周期。
